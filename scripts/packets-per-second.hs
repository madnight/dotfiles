...
packetsPerSecond i d  = do
    let readStrictSafe =
            Strict.readFile (packetsOSPath i d) `catch` exceptionHandler
    ...
    print (fromJust y - fromJust x)
        where -- error handling
            exceptionHandler e
              ...
              | isPermissionError e = error' "cannot access network interface \
                  \packets information: permission denied"
            fromJust (Just x) = x
            fromJust Nothing  = errorWithoutStackTrace "interface packet \
                \information malformed or non existing"
